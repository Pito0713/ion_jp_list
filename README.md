...
```
調整 GammmarPage 單字小卡樣式
調整 TextPage 單字小卡樣式
修正 HomePage 修正答案暫存問題
```
...
```
新增 共用 LoadingBN 載入按鈕

調整 各頁面 使用 Loading 導入共用按鈕
調整 TextPage 監聽 selectOption 有變動更新資料
調整 homePage 修正單字量不足 不顯示測驗題目
調整 homePage 選擇答案選擇顏色與 新增答題筆數標題
```
