...
VeeForm(v-slot="{ handleSubmit }" :validation-schema="schema" as="div")
  form(@submit="handleSubmit($event, () => $emit('submit-add'))" class='w-full flex-1')
    div(class='flex justify-center items-center flex-col')
      div(class='w-full flex-1')
        Card(class='flex justify-center items-center flex-col mt-4')
          div(class='flex w-full')
            div(class='flex flex-col w-1/2 mr-2')
              ...
              VeeField(name="text" :modelValue='propsTextInput' type="text" v-slot="{ field, value }")
                input(
          ...
          div(class='flex flex-row w-full')
            template(v-for='(item, index) in propsTagArray' :key='item')
              Tag(@click='$emit("handle-tag",index)' :class='item.active && "bg-primary-color text-white"')
        ...
        Card(class='mt-2.5 flex-col ')
          ...
          template(v-for='(input, index) in propsInputs' :key='index')
            div
