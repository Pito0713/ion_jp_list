          ...
          div(class='w-9/12 flex sm:flex-row flex-col justify-start sm:items-center items-start mb-1')
            div(class='flex flex-row justify-start items-start')
              div(class='flex')
                ...
                a(v-if='item.fileHiragana' class=' text-gray-500 text-xs font-medium mt-3') {{item.fileHiragana}}
                div(@click='$emit("copy-text",item.file)'  class='active:opacity-20 ml-1 mr-4 mt-3')
                  ImageFC(src='/img/item_copy.png' :width='16' :height='16' )
                a(v-if='item.fileTranslate' class=' text-base font-medium mt-2') {{item.fileTranslate}} 
