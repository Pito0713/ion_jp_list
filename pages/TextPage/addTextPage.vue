          ...
          div(class='w-full flex-1')
            Card(class='flex justify-center items-center flex-col')
              div(class='flex flex-col w-full')
                a(class='w-full text-xl mb-2 font-bold') {{$t('single_word')}}
                VeeField(name="text" type="text" v-slot="{ field }")
                  input(:placeholder="$t('please_enter_word')" v-model="textInput" class='mx-2.5 w-full' v-bind="field")
            ...
            Card(class='mt-2.5 flex-col')
              ...
              div(v-for='(input, index) in inputs' :key='index' class='flex justify-center items-center flex-row')
                div(class="grid grid-cols-8 gap-4 mb-2.5")
                  input(type='text', v-model='input.jpValue' :placeholder="('please_enter_word')" class="col-span-3")
                  input(type='text', v-model='input.chValue' :placeholder="('please_enter_translate')" class="col-span-3")
                  div(@click='remove(index)'  class='text-center col-span-2 custom-button') {{('delete')}}
